...
module.exports = class RemoveRoleCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      examples: ['removerole @ValiantWind @Moderator']
    });
  }
  async run(message, args) {
    ...
    if (member.roles.highest.position >= message.member.roles.highest.position)
      return this.sendErrorMessage(message, 0, 'You cannot remove a role from someone with an equal rank to yours or a higher rank than yours.');
    ...
    if (!role) 
      ...
      return this.sendErrorMessage(message, 0, 'User does not have the provided role how do you not know that smh');
    else {
      try {
        ...
        this.sendErrorMessage(message, 1, 'The role you provided is higher than my role :(. You should move my role above the role you want to remove', err.message);
      }
    }  
  }
};