...
module.exports = class PurgeBotCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      description: oneLine`
        Deleted all the messages from me on the provided channel.
        If no channel is given, the messages will be deleted from the current channel.
        You cannot delete more than 100 messages at a time nor can messages older than 2 week be deleted.
      `,
  ...
  async run(message, args) {
    ...
    Please mention a valid text channel or provide a valid text channel ID. The one you provided doesn't exist lol
    `);

    const amount = parseInt(args[0]);
    if (isNaN(amount) === true || !amount || amount < 0 || amount > 100)
      return this.sendErrorMessage(message, 0, 'Please provide a message value between 1 and 100');
    ...
    const prefix = require('../../config.json').prefix; // Get prefix
