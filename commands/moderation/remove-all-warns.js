...
module.exports = class ClearWarnsCommand extends Command {
  constructor(client) {
    super(client, {
      name: 'clearwarnings',
      aliases: ['clearwarns'],
      usage: 'clearwarnings <user mention/ID> [reason]',
  ...
  run(message, args) {
    ...
    if (member === message.member) 
      return this.sendErrorMessage(message, 0, 'You cannot clear your own warns nice try you bad member'); 
    if (member.roles.highest.position >= message.member.roles.highest.position)
      return this.sendErrorMessage(message, 0, 'You cannot clear the warns of someone with an equal rank to yours or a higher rank than yours.');
