...
module.exports = class SetNicknameCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      examples: ['setnickname @ValiantWind', 'setnickname @ValiantWind "Sussy Baka"']
    });
  }
  async run(message, args) {
    ...
    if (member.roles.highest.position >= message.member.roles.highest.position && member != message.member)
      ...
      You cannot change the nickname of someone with an equal rank to yours or a higher rank than yours.'
      `);

    if (!args[1]) return this.sendErrorMessage(message, 0, 'Please provide a nickname lmao');
    ...
    if (nickname.startsWith('"')) {
      ...
      if (!nickname.includes('"')) 
        return this.sendErrorMessage(message, 0, 'Please ensure the nickname is in quotes');
      nickname = nickname.slice(0, nickname.indexOf('"'));
      if (!nickname.replace(/\s/g, '').length) return this.sendErrorMessage(message, 0, 'Please provide a nickname lmao');
    }

    if (nickname.length > 32) {
      return this.sendErrorMessage(message, 0, 'Make sure the nickname is no larger than 32 characters or else the world will end');
