...
module.exports = class MuteCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      description: 'Mutes a user for the specified amount of time (maximum 2 weeks or 14 days).',
      type: client.types.MOD,
      clientPermissions: ['SEND_MESSAGES', 'EMBED_LINKS', 'MANAGE_ROLES'],
      userPermissions: ['MANAGE_ROLES'],
      examples: ['mute @ValiantWind 10s', 'mute @ValiantWind 30m stop spamming smh']
    });
  }
  async run(message, args) {
    ...
    if (member === message.member)
      return this.sendErrorMessage(message, 0, 'You cannot mute yourself why would you even like to do that lol');
    if (member === message.guild.me) return this.sendErrorMessage(message, 0, 'Nice try. You cannot mute me');
    if (member.roles.highest.position >= message.member.roles.highest.position)
      return this.sendErrorMessage(message, 0, 'You cannot mute someone with an equal rank to yours or a higher rank than yours.');
    if (!args[1])
      return this.sendErrorMessage(message, 0, 'Please enter a length of time of 14 days or less.');
    let time = ms(args[1]);
    ...
    if (member.roles.cache.has(muteRoleId))
      return this.sendErrorMessage(message, 0, 'The member is already muted how do you not know that lol');
