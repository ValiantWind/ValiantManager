...
module.exports = class KickCommand extends Command {
  ...
  async run(message, args) {
    ...
    if (member === message.member) 
      return this.sendErrorMessage(message, 0, 'You cannot kick yourself why would you even want to do that lol'); 
    if (member.roles.highest.position >= message.member.roles.highest.position)
      return this.sendErrorMessage(message, 0, 'You cannot kick someone with an equal rank to yours or a higher rank than yours.');
    if (!member.kickable) 
